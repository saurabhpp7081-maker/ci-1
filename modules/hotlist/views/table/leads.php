...
return $this
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $aColumns = array_merge($aColumns, [
            'COALESCE(' . db_prefix() . 'hotlist_lead_rounds.company_name, ' . db_prefix() . 'leads.company) as hotlist_company_name',
        ...
        $join = [
            ...
            'LEFT JOIN ' . db_prefix() . 'hotlist_lead_rounds ON ' . db_prefix() . 'hotlist_lead_rounds.lead_id = ' . db_prefix() . 'leads.id',
        ];
        ...
        foreach ($rResult as $aRow) {
            ...
            $hrefAttr = 'href="javascript:void(0);" onclick="window.init_hotlist_lead(' . $aRow['id'] . '); return false;"';
            ...
            if (! $locked) {
                $nameRow .= ' | <a href="javascript:void(0);" onclick="window.init_hotlist_lead_edit(' . $aRow['id'] . '); return false;">' . _l('edit') . '</a>';
            }

            if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
                $nameRow .= ' | <a href="' . admin_url('hotlist/delete/' . $aRow['id']) . '" class="_delete text-danger">' . _l('delete') . '</a>';
            }
            $nameRow .= '</div>';
            ...
            $row[] = e($aRow['hotlist_company_name']);
            ...
            if ($aRow['assigned'] != 0) {
                ...
                $assignedOutput = '<span data-toggle="tooltip" data-title="' . $full_name . '">' . staff_profile_image($aRow['assigned'], [
